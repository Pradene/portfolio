...
class InputManager {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      // If focus is locked, don't do anything with mouse movement
    ...
    document.addEventListener("keydown", (e) => {
      if (["ArrowRight", "ArrowLeft", "Tab"].includes(e.key)) {
        ...
        this.clearFocus()
        this.cursor.setTargetPosition(this.actualMousePosition.x, this.actualMousePosition.y)
      }
    })
	}
    ...
    // Tell the cursor to animate around this element
    const elementSize = element.getSize()
    const elementPosition = element.getPosition()

    // Offset from cursor's current position
    const x = elementPosition.x + elementSize.width / 2
    const y = elementPosition.y + elementSize.height / 2
    this.cursor.setTargetPosition(x, y)
	}
	
	/**
	 * Locks the focus so mouse movement doesn't affect it
	 */
	lockFocus() {
	...
	getFocusableAtPosition(position) {
		for (let i = 0; i < this.focusables.length; i++) {
			...
			if (Math.abs(focusablePos.x - position.x) < 2 && Math.abs(focusablePos.y - position.y) < 2) {
	...
	navigate(key) {
		...
		const target = this.focusables[this.currentIndex]
